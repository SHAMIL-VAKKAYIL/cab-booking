...
import { publishRideCancelled, publishRideRequested } from '../../events/producer/ride-request.producer';
...
export class RiderService {
    ...
    async rateDriver(input: CreateRatingInput) {
        try {
            ...
            const trip = await db
                .select()
                .from(rideHistory)
                .where(eq(rideHistory.tripId, tripId))
    ...
    async requestRide(input: RequestRideInput) {
        try {
            const { riderId, pickupLat, pickupLng, pickupAddress, dropoffLat, dropoffLng, dropoffAddress, vehicleType } = input;
            ...
            logger.error({ error }, 'Failed to request ride');
            throw error;
        }
    }
